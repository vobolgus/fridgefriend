...
def derive_shopping_list(plan: PlanResult, inventory: list[InventoryItem]) -> list[ShoppingItem]:
    ...
    for ingredient_name, requirement in sorted(required_by_ingredient.items()):
        ...
        items.append(
            ShoppingItem(
                ...
                reason="not in inventory" if available_quantity == 0 else "insufficient quantity",
            ),
        )
